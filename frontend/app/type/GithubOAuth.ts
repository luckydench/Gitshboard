 ...
 */
export interface StrictGithubOAuthParams  
extends Omit<GithubOAuthParams, 'state'>,  Record<string, string> {
    scope : GitHubScope;
}
// strict 타입이기 때문에 Record로 undefined가 불가능하게 강제시킴
// state는 CSRF 공격 방지용으로 사용되지만, 현재 구현에서는 생략시켰음. 필요에 따라 추가할 수 있음
